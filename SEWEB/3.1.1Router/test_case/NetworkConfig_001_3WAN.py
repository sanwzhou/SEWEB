...
logger = LogGen(Logger = 'NetworkConfig_001_3WAN').getlog()

class Config_3WAN(unittest.TestCase):
    ...
    def test_000_getReady(self):
        ...
        try:
            ...
            Select(selPortNumber).select_by_value('3')
            time.sleep(2)
            try:
                wan_config.find_cfm_ok()
            except NoSuchElementException:
                logger.info(u'WAN口数量已为3')
            else:
                time.sleep(30)
                i = 0
                while i < 20:
        ...
        self.driver.quit()
        #2 调整wan2\wan3 与上联口互通
        swconfig.test_changeWAN3(self)
    ...
    def test_001_static(self):
        ...
        while n < 30:
            ...
            print(str(list_connectState1), str(list_connectState2), str(list_connectState3))
        ...
        print('wan1route:', wan1route, 'wan2route:', wan2route, 'wan3route:', wan3route)
    ...
    def test_002_staticSW(self):
        ...
        RetryTimes3 = str(wan_config.getAttribute_byName(wan_config.RetryTimes3v,'value'))
        DestIP3 = str(wan_config.getAttribute_byName(wan_config.DestIP3v,'value'))
        ...
        print('wan1route_2:', wan1route_2, 'wan2route_2:', wan2route_2, 'wan3route_2:', wan3route_2)
        time.sleep(10)  # 等待检测，路由切换
        x = 0
        while x < 60:
            tn = telnetlib.Telnet(host=hostip, port=port,timeout=10)
