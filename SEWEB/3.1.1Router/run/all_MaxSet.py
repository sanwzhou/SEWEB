...
while testCount <= 1:
    ...
    # mini = True
    mini = True
    # mini = ProductModel_test == '0' or (
    #             ProductModel_test == '1' and testCount % 2 == 1)  # (不启用产品型号测试/启用产品型号测试且测试次数为奇数次)
    # mini = ( testCount <=3) # 测试次数 小于多少

    # 测试项为 产品型号 条件判断
    Model = True
    # Model = False
    # Model = (ProductModel_test == '1' and testCount % 2 == 0)  # (启用产品型号测试且测试次数为偶数次)
    # Model = ( testCount ==4 )  # 测试次数 等于多少/
    ...
    if True:
        ...
        if testCount > 1:
            if ('SoftVersion%s' % testCount) not in getweb(ReplaceVersion):
